...
namespace d2
{
    ...
    Element::EventListenerState::Dep Element::EventListenerState::value() const
    {
        return _value;
    }
    ...
    template<typename... Argv> void Element::EventListenerState::invoke(Argv&&... args)
    {
    ...
    Element::write_flag Element::_contextual_change(write_flag type) const
    {
        ...
        if (type & (InternalState::DimensionsWidthUpdated | InternalState::DimensionsHeightUpdated))
            return (bool(rep & ContextualXPos) * WriteType::LayoutXPos) |
    ...
    Element::_push_listener(State event, EventListenerState::Dep value, event_callback callback)
    {
        auto& l = _subscribers.emplace_back(
            std::make_shared<EventListenerState>(
                shared_from_this(), _subscribers.size(), value, event, std::move(callback)
            )
        );
        if (event == State::Clicked || event == State::Focused)
            _cursor_sink_listener_cnt++;
        if (event == State::Event || event == State::RcEvent)
            _dynamic_input_listener_cnt++;
    ...
    void Element::_unmute_listener(EventListenerState::ptr listener)
    {
        ...
        if (listener->event() == State::Clicked || listener->event() == State::Focused)
            _cursor_sink_listener_cnt++;
        if (listener->event() == State::Event || listener->event() == State::RcEvent)
            _dynamic_input_listener_cnt++;
    ...
    void Element::_destroy_listener(EventListenerState::ptr listener)
    {
        ...
        for (auto it = _subscribers.begin(); it != _subscribers.end();)
        {
            ...
            {
                _subscribers.erase(beg, _subscribers.end());
                return;
            }
            else
                ++it;
    ...
    void Element::_trigger(State event, bool value)
    {
        for (std::size_t i = 0; i < _subscribers.size(); i++)
        {
            ...
            if (l != nullptr && l->event() == event &&
                (l->value() == EventListenerState::Dep::Any ||
                 (value ? l->value() == EventListenerState::Dep::On
                        : l->value() == EventListenerState::Dep::Off)))
                l->invoke(traverse());
        }
    }
    ...
    void Element::_trigger_event(in::InputFrame& frame, bool recursive)
    {
        _tmp_frame = &frame;
        setstate(RcEvent, recursive);
        setstate(State::Event, true);
        _event_impl(frame);
        setstate(State::Event, false);
        setstate(RcEvent, false);
        _tmp_frame = nullptr;
        in::internal::InputFrameView::from(&frame).apply_consume();
    }
    ...
    {
        if (_buffer.empty() && parent() != nullptr)
        {
            const auto [width, height] = box();
            const auto [x, y] = position();
            return PixelBuffer::View{parent()->_fetch_pixel_buffer_impl(), x, y, width, height};
        }
        else
        {
            return PixelBuffer::View{&_buffer, 0, 0, _buffer.width(), _buffer.height()};
        }
    }
    ...
    in::InputFrame& Element::input_frame() const
    {
        if (_tmp_frame == nullptr)
            D2_THRW("Element is not current receiving input (frame invalid)");
        return *_tmp_frame;
    }
    ...
    void Element::setstate(State state, bool value)
    {
        if (getstate(state) != value)
        {
            ...
            {
                if (!value)
                    _parent = std::weak_ptr<ParentElement>();
                _reset_depth();
            }
    ...
    void Element::_setparent(pptr parent)
    {
        _parent = parent;
    }
    std::size_t Element::depth()
    {
        return _depth;
    }
    ...
    {
        return _cursor_sink_listener_cnt || _dynamic_input_listener_cnt || _provides_input_impl();
    }
    bool Element::provides_input() const
    {
        return getstate(Display) && (_provides_input_impl() || _dynamic_input_listener_cnt);
    }
    bool Element::needs_update() const
    {
        return _internal_state & WasWritten || _internal_state & CachePolicyVolatile;
    }
    ...
    {
        return _push_listener(
            event, value ? EventListenerState::Dep::On : EventListenerState::Dep::Off, callback
        );
    }
    Element::EventListener Element::listen(State event, event_callback callback)
    {
        return _push_listener(event, EventListenerState::Dep::Any, callback);
    }
    ...
    {
        auto value = _layout_impl(type);
        switch (type)
        {
        case Layout::X:
        case Layout::Y:
            value.setmods(Unit::Mods::_Position);
            break;
        case Layout::Width:
        case Layout::Height:
            value.setmods(Unit::Mods::_Dimensions);
            break;
        }
        return value;
    }
    ...
    {
        _internal_state |=
            (PositionXUpdated | PositionYUpdated | DimensionsWidthUpdated |
             DimensionsHeightUpdated);
    }
    void Element::accept_layout(enum Layout type) const
    {
        ...
        {
        case Layout::X:
            _internal_state |= PositionXUpdated;
            break;
        case Layout::Y:
            _internal_state |= PositionYUpdated;
            break;
        case Layout::Width:
            _internal_state |= DimensionsWidthUpdated;
            break;
        case Layout::Height:
            _internal_state |= DimensionsHeightUpdated;
            break;
        }
    }
    ...
    bool Element::contextual_layout(enum Layout type) const
    {
        ...
        {
        case Layout::X:
            return rep & ContextualXPos;
        case Layout::Y:
            return rep & ContextualYPos;
        case Layout::Width:
            return rep & ContextualWidth;
        case Layout::Height:
            return rep & ContextualHeight;
        }
    }
    bool Element::relative_layout(Layout type) const
    {
        ...
        {
        case Layout::X:
            return rep & RelativeXPos;
        case Layout::Y:
            return rep & RelativeYPos;
        case Layout::Width:
            return rep & RelativeWidth;
        case Layout::Height:
            return rep & RelativeHeight;
        }
    }
    ...
    Element::Frame Element::frame()
    {
        ...
        if (_internal_state & CachePolicyBypass)
        {
            if (needs_update())
            {
        ...
        else
        {
            ...
            if (needs_update())
            {
                if (const auto [width, height] = box(); width > 0 && height > 0)
                {
                    const auto ab = !_provides_buffer_impl() || parent() == nullptr;
                    _frames_reused = 0;
                    _update_style_impl();
                    if (((_internal_state & CachePolicyStatic) ||
                         ((_internal_state & CachePolicyDynamic) && _frames_redrawn == 1)) &&
                        ab)
                    {
                        const auto [bwidth, bheight] = _reserve_buffer_impl();
                        _buffer.set_size(bwidth, bheight);
                    ...
                    D2_ASSERT(!(_buffer.empty() && parent() == nullptr))
                    _internal_state |= IsBeingRendered;
                    _signal_update(WasWritten);
                    _frame_impl(_fetch_pixel_buffer_impl());
                    _internal_state &= ~IsBeingRendered;

                    if ((_internal_state & CachePolicyStatic) && !_buffer.empty())
                    {
                        _buffer.compress();
                        _frames_reused = 10;
                    }
            ...
            else if (!(_buffer.empty() || _buffer.compressed()) && ++_frames_reused == 10)
            {
                _buffer.compress();
            }
            else
    ...
    Element::Position Element::mouse_object_space() const
    {
        ...
        const auto [x, y] = _state_ptr->context()->input()->mouse_position();
        return {x, y};
    }
    ...
    namespace internal
    {
        ...
        void ElementView::signal_context_change_sub(
            Element::write_flag type, unsigned int prop, Element::ptr element
        )
        {
        ...
        void ElementView::signal_context_change(
            Element::write_flag type, unsigned int prop, Element::ptr element
        )
        {
        ...
        {
            return _ptr->_trigger_event(frame, recursive);
        }

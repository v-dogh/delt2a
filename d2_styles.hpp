...
namespace d2::style
{
    namespace impl
    {
        ...
        concept chained = requires { (Type::last_offset_); };

        template<typename Chain> struct ResolveChain
        {
            static constexpr auto offset = 0;
        };
        template<chained Chain> struct ResolveChain<Chain>
        {
            static constexpr auto offset = Chain::last_offset_;
        };

        template<D2_UAI_INTERFACE_TEMPL First, D2_UAI_INTERFACE_TEMPL... Rest> struct LastImpl
        {
            template<std::size_t Prop> using type = LastImpl<Rest...>::template type<Prop>;
        };
        template<D2_UAI_INTERFACE_TEMPL Last> struct LastImpl<Last>
        {
            template<std::size_t Prop> using type = Last<Prop>;
        };
        ...
        struct OffsetHelperImpl
        {
            using type = std::conditional<
                ...
                Current,
                typename OffsetHelperImpl<Search, Next<Current::offset>, Next, Rest...>::type
            >::type;
        };
        template<
            D2_UAI_INTERFACE_TEMPL Search,
        ...
        struct OffsetHelper
        {
            using result = OffsetHelperImpl<Current, First<BaseOffset>, First, Rest...>;
    ...
    // UAI provides read/write access to styles (member variables) of objects that derive from it
    // (elements) It handles proper synchronization and signals, and manages the process of
    // assigning unique ID's to properties for elements at compile time It also facilitates the
    // ability for objects to derive from other objects through chaining I.e. the topmost UAI is
    // aware of the lower UAI and delegates their property accesses to it Access to variables
    // through UAI is also automatically synchronized I.e. accesses from separate threads will be
    // queued on the main thread by default Main thread reads/writes should mostly be zero overhead
    // The ILayout interface is special as every UAI needs to have one in the base element

    template<std::size_t PropBase> struct ILayout;
    ...
    >
    class UniversalAccessInterface : public Chain,
                                     public Interface<impl::ResolveChain<Chain>::offset>,
                                     public Interfaces<impl::OffsetHelper<
    ...
    private:
        ...
        using last_interface = impl::LastImpl<Interface, Interfaces...>::template type<Prop>;
        static constexpr std::size_t base_offset_ = impl::ResolveChain<Chain>::offset;
        static constexpr std::size_t last_offset_ =
            impl::OffsetHelper<base_offset_, last_interface, Interface, Interfaces...>::offset +
            last_interface<0>::count;
        static constexpr std::size_t chain_base_ = impl::ResolveChain<Chain>::offset;
        static constexpr bool chain_ = impl::chained<Chain>;

        // Templates

        template<std::size_t Offset, D2_UAI_INTERFACE_TEMPL Search, D2_UAI_INTERFACE_TEMPL... Rest>
        ...
        struct SearchInterfaceImpl<Offset, Search, Current, Rest...>
        {
            using type = std::conditional<
                ...
                Current<Offset>,
                typename SearchInterfaceImpl<Current<Offset>::offset, Search, Rest...>::type
            >::type;
        };
        template<std::size_t Offset, D2_UAI_INTERFACE_TEMPL Search, D2_UAI_INTERFACE_TEMPL Current>
        struct SearchInterfaceImpl<Offset, Search, Current>
        {
            using type = std::
                conditional<std::is_same_v<Search<0>, Current<0>>, Current<Offset>, void>::type;
        };
        template<D2_UAI_INTERFACE_TEMPL Type> struct FindInterface
        {
            using type = SearchInterfaceImpl<base_offset_, Type, Interface, Interfaces...>;
        };

        template<std::size_t Search, typename Current, D2_UAI_INTERFACE_TEMPL... Rest>
        ...
        {
            using type =
                std::conditional < Search >=
                Current::base&& Search<
                    Current::offset,
                    Current,
                    typename SearchPropertyOwnerImpl<Search, Next<Current::offset>, Rest...>::type
                >::type;
        };
        template<std::size_t Search, typename Current>
        struct SearchPropertyOwnerImpl<Search, Current>
        {
            using type = std::conditional < Search >=
                         Current::base&& Search<Current::offset, Current, void>::type;
        };
        template<std::size_t BaseOffset, std::size_t Search> struct SearchPropertyOwner
        {
            using type =
                SearchPropertyOwnerImpl<Search, Interface<BaseOffset>, Interfaces...>::type;
        };
    private:
        ...
        template<property Property> auto _int_get_vals()
        {
            static_assert(Property <= last_offset_, "Invalid Property");
            using interface = SearchPropertyOwner<base_offset_, Property>::type;
            return interface::template get<Property - interface::base>();
        }
        template<property Property> auto _int_get()
        {
            auto [ptr, _] = _int_get_vals<Property>();
            return *ptr;
        }
        template<property Property> auto* _int_getref()
        {
            auto [ptr, _] = _int_get_vals<Property>();
            return ptr;
        }

        template<property Property> auto _int_get_vals_chained()
        {
            if constexpr (chain_)
            {
                if constexpr (Property < chain_base_)
                {
                    return static_cast<Chain&>(static_cast<Base&>(*this))
        ...
        template<property Property> auto _int_get_synced()
        {
            const auto ctx = _context();
            if (ctx->is_synced())
            {
    ...
    protected:
        ...
        {
            if ((this->parent() &&
                 !this->parent()->getistate(Element::InternalState::IsBeingInitialized)) ||
                force)
            {
        ...
        virtual void* _has_interface_impl(std::size_t id) override
        {
            if constexpr (chain_)
            {
                auto* ptr = static_cast<Chain::data*>(static_cast<Base*>(this))
        ...
        virtual std::shared_ptr<IOContext> _context_impl() const override
        {
            return _context();
        }
    ...
    public:
        ...
        template<property Property> auto get()
        {
            if constexpr (chain_)
            {
                if constexpr (Property < chain_base_)
                {
                    return static_cast<Chain&>(static_cast<Base&>(*this)).template get<Property>();
                }
                else
                {
        ...
        template<property Property> auto* getref()
        {
            if constexpr (chain_)
            {
                if constexpr (Property < chain_base_)
        ...
        template<property Property, typename Func> auto apply_get(Func&& func)
        {
            const auto _ = _base()->shared_from_this();
            const auto ctx = _context();
            if (ctx->is_synced())
    ...
    class UniversalAccessInterfaceProto
        : public UniversalAccessInterface<Elem, Base, Interface, Interfaces...>
    {
    private:
        using base = UniversalAccessInterface<Elem, Base, Interface, Interfaces...>;
        static_assert(
            std::is_same_v<Interface<0>, ILayout<0>> ||
                (std::is_same_v<Interfaces<0>, ILayout<0>> || ...),
    ...
    protected:
        ...
        {
            return (Element::RelativeXPos * this->x.relative()) |
    ...
    template<typename Base, D2_UAI_INTERFACE_TEMPL Interface, D2_UAI_INTERFACE_TEMPL... Interfaces>
    ...
    namespace impl
    {
        template<typename Type> struct BitFlagPtrWrapper
        {
            Type* ptr{nullptr};
            Type mask{0x00};
        ...
        template<template<std::size_t> typename> struct XAllocInterface
        {
        protected:
...
#define D2_UAI_INTERFACE(_name_, _opts_, _fields_, _props_, _links_)                               \
    ...
    public:                                                                                        \
        template<                                                                                  \
            ...
            D2_UAI_INTERFACE_TEMPL... Interfaces                                                   \
        >                                                                                          \
        friend class ::d2::style::UniversalAccessInterface;                                        \
        template<                                                                                  \
            D2_UAI_INTERFACE_TEMPL Search,                                                         \
            typename Current,                                                                      \
            D2_UAI_INTERFACE_TEMPL CurrentType,                                                    \
            ...
            D2_UAI_INTERFACE_TEMPL... Rest                                                         \
        >                                                                                          \
        friend class ::d2::style::impl::OffsetHelperImpl;                                          \
        template<                                                                                  \
            std::size_t BaseOffset,                                                                \
            D2_UAI_INTERFACE_TEMPL Current,                                                        \
            D2_UAI_INTERFACE_TEMPL First,                                                          \
            D2_UAI_INTERFACE_TEMPL... Rest                                                         \
        >                                                                                          \
        friend class ::d2::style::impl::OffsetHelper;                                              \
        friend class ::d2::style::UniversalAccessInterfaceBase;                                    \
        _props_;                                                                                   \
    };                                                                                             \
...
#define D2_UAI_PROP(_prop_, _var_, ...)                                                            \
    else if constexpr (Property == unsigned(_prop_)) return std::make_pair(                        \
        &_var_, static_cast<unsigned char>(__VA_ARGS__)                                            \
    );
#define D2_UAI_PROP_VAR(...) __VA_ARGS__
    ...
    D2_UAI_INTERFACE(
        ...
        D2_UAI_OPTS(),
        D2_UAI_FIELDS(
            HPUnit x{Unit::Auto}; VPUnit y{Unit::Auto}; HDUnit width{Unit::Auto};
            VDUnit height{Unit::Auto};
            char zindex{0};
        ),
        D2_UAI_PROPS(X, Y, Width, Height, ZIndex),
        D2_UAI_LINK(D2_UAI_PROP(X, x, LayoutXPos) D2_UAI_PROP(Y, y, LayoutYPos)
    ...
    D2_UAI_INTERFACE(
        ...
        D2_UAI_FIELDS(
            string text{}; Alignment alignment{Alignment::Center};
            unsigned char text_options{Paragraph};
        ),
        D2_UAI_PROPS(
            Value, TextAlignment, TextOptions, TextPreserveWordBoundaries, TextHandleNewlines
        ),
        D2_UAI_LINK(
            D2_UAI_PROP(Value, text, Style) D2_UAI_PROP(TextAlignment, alignment, Style)
                D2_UAI_PROP(TextOptions, text_options, Style) D2_UAI_PROP(
                    TextPreserveWordBoundaries,
                    D2_UAI_PROP_VAR(impl::uchar_bit_ptr(&text_options, PreserveWordBoundaries)),
                    Style
                )
                    D2_UAI_PROP(
                        TextHandleNewlines,
                        D2_UAI_PROP_VAR(impl::uchar_bit_ptr(&text_options, HandleNewlines)),
                        Style
                    )
        )
    )

    template<typename Type, typename... Argv> struct Responsive
    {
        std::function<void(d2::TreeIter<Type>, Argv...)> on_submit{};

        template<uai_property Property> auto get()
        {
            if constexpr (Property == 0)
                return std::make_pair(&on_submit, static_cast<unsigned char>(0x00));
            else
            {
                static_assert(false, "Invalid Property");
                return std::make_pair(nullptr, static_cast<unsigned char>(0x00));
    ...
    template<typename Type, typename... Argv> struct IResponsive
    {
        template<std::size_t PropBase>
        struct type : Responsive<Type, Argv...>, impl::InterfaceHelper<type, PropBase, 1>
        {

...
namespace d2
{
    ...
    public:
        ...
        static auto build_sub(
            const std::string& name, TreeState::ptr state, d2::Element::pptr root, Argv&&... args
        )
        {
            auto s = TreeState::make<State>(
                state->root(), nullptr, nullptr, std::forward<Argv>(args)...
            );
            auto core = d2::Element::make<Root>(name, s);
        ...
        template<typename... Argv> static auto build(IOContext::ptr ctx, Argv&&... args)
        {
            auto state = TreeState::make<State>(nullptr, nullptr, ctx, std::forward<Argv>(args)...);
            state->set_root(d2::Element::make<Root>("", state));

...
namespace d2::fb
{
    ...
    private:
        ...
        public:
            ...
            static auto
            from(bool (*callback)(FragmentedBuffer*, void*, void*, Type), Argv&&... args)
            {
                return Action(callback, new std::tuple<Argv...>(std::forward<Argv>(args)...));
            }
            template<typename Type>
            static auto from(bool (*callback)(FragmentedBuffer*, void*, void*, Type), Type* ptr)
            {
                return Action(callback, ptr);
            }

            template<typename State> static auto* alloc_state(State value, std::size_t len = 0)
            {
                auto* buffer =
                    ::operator new[](sizeof(State) + len, std::align_val_t(alignof(State)));
            ...
            template<typename State> static void delete_state(void* ptr)
            {
                static_cast<State*>(ptr)->~State();
                ::operator delete[](ptr, std::align_val_t(alignof(State)));
            }

            Action(bool (*callback)(FragmentedBuffer*, void*, void*, Type), void* data = nullptr);
    ...
    private:
        ...
        bool _enabled_history{true};
        PtrState _ptr{};

        // Access

        void _iterate(std::function<It(Locale::Value)> callback, std::size_t pos) const;
        void _iterate_ptr(std::function<It(Locale::Value)> callback) const;
        void _iterate_raw(std::function<bool(std::string_view)> callback) const;
    ...
    public:
        ...
        struct : Namespace
        {
            template<typename Type> void locale()
            {
                _buf->_set_locale(std::make_unique<Type>());
            }

            void setup_buffer(std::string value)
            {
                _buf->_setup_buffer(value);
            }
            void setup_view(std::string_view value)
            {
                _buf->_setup_view(value);
            }

            void iterate(std::function<It(Locale::Value)> callback, std::size_t pos = 0) const
            {
                _buf->_iterate(std::move(callback), pos);
            }
            void iterate_ptr(std::function<It(Locale::Value)> callback) const
            {
                _buf->_iterate_ptr(std::move(callback));
            }
            void iterate_raw(std::function<bool(std::string_view)> callback) const
            {
                _buf->_iterate_raw(std::move(callback));
            }
        } buf{this};
        struct : Namespace
        {
            ...
            void left(std::size_t off = 1)
            {
                _buf->_move_left(off);
            }
            void right(std::size_t off = 1)
            {
                _buf->_move_right(off);
            }
            void up(std::size_t off = 1)
            {
                _buf->_move_up(off);
            }
            void down(std::size_t off = 1)
            {
                _buf->_move_down(off);
            }
            ...
            {
                return _buf->_ptr.abs;
            }
            std::size_t line_offset() const
            {
                return _buf->_ptr.line_off;
            }
        } ptr{this};
        struct : Namespace
        {
        ...
        struct : Namespace
        {
            void enable()
            {
                _buf->_enabled_history = true;
            }
            void disable()
            {
                _buf->_enabled_history = false;
            }
            void reapply()
            {
                _buf->_redo_action();

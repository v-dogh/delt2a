...
namespace d2::ex
{
    ...
    class FolderView : public style::UAIC<dx::Box, FolderView, style::IKeyboardNav>,
    ...
    public:
        ...
        using data = style::UAIC<dx::Box, FolderView, style::IKeyboardNav>;
    ...
    protected:
        ...
        virtual void _state_change_impl(State state, bool value) override
        {
            if (state == State::Created && value)
            {
                ...
                _scrollbar = std::static_pointer_cast<VerticalSlider>(
                    create(Element::make<dx::VerticalSlider>("", this->state())).shared()
                );
                ...
                D2_STYLE(
                    ForegroundColor,
                    D2_DYNAVAR(
                        WidgetTheme, wg_fg_button(), value.extend(d2::charset::slider_vertical)
                    )
                )
                D2_CONTEXT_END

                internal::ElementView::from(_scrollbar).signal_write(Masked);
            ...
            else if (state == State::Clicked && value)
            {
                ...
                if (x < width - 1 && _scrollbar->absolute_value() + idx <
                                         (_use_filtered ? _filtered_view.size() : _entries.size()))
                {
                    if (idx + src != _focused_idx)
                    {
                        _focused_idx = idx + src;
                        _core(this->state())->sselect(_focused_entry()->path);
                        _signal_write(Style);
                    }
                    else
                    {
                        _core(this->state())->rselect(_focused_entry()->path);
                    }
                }
            }
        ...
        virtual void _event_impl(in::InputFrame& frame) override
        {
            ...
            if (frame.had_event(in::Event::ScrollWheelMovement))
            {
                if (getstate(RcHover))
                {
                    ...
                    _scrollbar->reset_absolute(_range_idx + scroll);
                }
            }
            if (frame.had_event(in::Event::KeyInput))
            {
                    ...
                    frame.active(in::special::ArrowRight, in::mode::Hold))
                {
                    ...
                    frame.active(in::special::ArrowLeft, in::mode::Hold)
                )
                {
                    if (_focused_idx == -1)
                        _focused_idx = 0;
                ...
                else if (frame.active(in::special::Enter, in::mode::Hold))
                {
                    if (_focused_idx != -1)
                    {
                        _core(this->state())->rselect(_focused_entry()->path);
        ...
        virtual void _update_layout_impl() override
        {
            ...
            _scrollbar->set<dx::Slider::SliderWidth>(std::max<std::size_t>(
                1,
                (full_height && full_height > height) ? (float(height) / full_height) * height
                                                      : height
            ));
        ...
        {
            return _use_filtered ? _filtered_view[_focused_idx] : &_entries[_focused_idx];
        }
        void _render_entry(FileEntry* ptr, int offset, bool focused, PixelBuffer::View buffer)
        {
            const auto [width, height] = box();
            const auto px = Pixel::combine(
                data::foreground_color, data::focused_color.alpha(focused ? 0.8f : 0.f)
            );
            TextHelper::_render_text_simple(ptr->value, px, {0, offset}, {width - 1, 1}, buffer);
            if (ptr->value.size() < width)
            {
                buffer.fill(px, ptr->value.size(), offset, width - ptr->value.size() - 1, 1);
            }
        }
    public:
        void filter(const string& reg)
        {
            ...
            {
                if (_settings & ShowHidden)
                    _filtered_view.reserve(_entries.size());
                else
                    _filtered_view.reserve(_entries.size() * 0.5);
                for (decltype(auto) it : _entries)
                {
                    if (!(_settings & ShowHidden) && it.path.starts_with('.'))
                        continue;
                    if (std::smatch match; std::regex_search(it.path, match, std::regex(reg)))
                    {
                        ...
                        _entries.push_back(
                            {.path = filename,
                             .value = std::format(
                                 ...
                                 filename,
                                 (is_dir ? "dir"
                                         : (has_fsize ? std::format("file:{}", _memory_units(fsize))
                                                      : "file")),
    ...
    D2_STYLE(
        OnSubmit,
        [](TreeIter<Button> ptr)
        { std::static_pointer_cast<FilesystemExplorerWindow>(_core(ptr->state()))->close(); }
    )
    D2_ELEM_END
    ...
    D2_STYLE(ForegroundColor, D2_VAR(WidgetTheme, wg_text()))
    D2_STYLE(FocusedColor, D2_VAR(WidgetTheme, wg_hbg_button()))
    D2_STYLE(OnSubmit, [](TreeIter<Button> ptr) { _core(ptr->state())->submit_soft(); })
    D2_ELEM_END
    D2_ELEM(Button)
    D2_STYLE(Value, "<-")
    D2_STYLE(X, 1.0_px)
    D2_STYLE(ForegroundColor, D2_VAR(WidgetTheme, wg_text()))
    D2_STYLE(FocusedColor, D2_VAR(WidgetTheme, wg_hbg_button()))
    D2_STYLE(OnSubmit, [](TreeIter<Button> ptr) { _core(ptr->state())->backwards(); })
    D2_ELEM_END
    D2_ELEM(Button)
    D2_STYLE(Value, "->")
    D2_STYLE(X, 4.0_px)
    D2_STYLE(ForegroundColor, D2_VAR(WidgetTheme, wg_text()))
    D2_STYLE(FocusedColor, D2_VAR(WidgetTheme, wg_hbg_button()))
    D2_STYLE(OnSubmit, [](TreeIter<Button> ptr) { _core(ptr->state())->forwards(); })
    D2_ELEM_END
    D2_ELEM(Text, info)
    D2_STYLE(ForegroundColor, D2_VAR(WidgetTheme, wg_text()))
    ...
    D2_STYLE(
        OnSubmit,
        [](TreeIter<Button> ptr, const string& filename) { _core(ptr->state())->rselect(filename); }
    )
    D2_ELEM_END
    D2_ELEM(Button)
    ...
    D2_STYLE(
        ...
        {
            const auto path =
                (*(_pcore(ptr->state())->traverse() / "search").as<Input>()).get<Input::Value>();
            auto folder = (_pcore(ptr->state())->traverse() / "folder").as<FolderView>();
            folder->filter(path);
    ...
    D2_STYLE(
        OnSubmit,
        [](TreeIter<Checkbox> ptr, bool value)
        {
            (ptr->state()->core()->traverse() / "folder")
                .as<FolderView>()
    ...
    namespace impl
    {
        ...
        void FilesystemExplorerBase::rselect(const string& filename)
        {
            ...
            else
            {
                ...
                {
                    history_.erase(history_.begin() + (history_.size() - rcnt_), history_.end());
                    rcnt_ = 0;
                }
        ...
        void FilesystemExplorerBase::setpath(std::filesystem::path path)
        {
            ...
            if (auto pstr = path.string(); pstr.size() >= max - 8)
            {
    ...
    void FilesystemExplorerWindow::_state_change_impl(State state, bool value)
    {
        if (state == State::Created && value)
        {
            ...
            data::set<VirtualBox::BarColor>(
                style::dynavar<[](const px::foreground& value) { return value.extend('.'); }>(
    ...
    void FilesystemExplorerWindow::_submit_path()
    {
        if (data::on_submit)
            data::on_submit(
                std::static_pointer_cast<FilesystemExplorerWindow>(shared_from_this()), path()
            );
        close();
    }
    ...
    {
        set<VirtualBox::Title>(std::format("<{}>", pstr));
        at("folder").as<FolderView>()->setpath(path());
        _update_results();
    }
    ...
    void FilesystemExplorer::_submit_path()
    {
        if (data::on_submit)
            data::on_submit(
                std::static_pointer_cast<FilesystemExplorer>(shared_from_this()), path()
            );
    }
    ...
    {
        at("search").as<dx::Input>()->set<dx::Input::Hint>(pstr);
        at("folder").as<FolderView>()->setpath(path());
        _update_results();
    }
} // namespace d2::ex

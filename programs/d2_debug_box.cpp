    ...
    D2_TREE_DEFINE(debug)
        ...
        D2_ELEM(FlowBox)
            ...
            D2_ELEM(Switch)
                ...
                D2_STYLE(
                    OnChangeValues,
                    [](d2::TreeIter<Switch> ptr, d2::string n, d2::string o)
                    {
            ...
            D2_ELEM(Text)
                ...
                D2_STYLE(Value, " ")
                D2_STYLE(X, 5.0_pxi)
                D2_INTERP_TWOWAY_AUTO(Hovered, 500, Linear, ForegroundColor, d2::colors::b::turquoise);
                D2_INTERP_GEN_AUTO(
                    1000, Sequential, Value, std::vector<d2::string>{"⠇", "⠋", "⠙", "⠸", "⢰", "⣠", "⣄", "⡆"}
                );
                D2_ON(Clicked)
                    state->context()->suspend(!state->context()->is_suspended());
                D2_ON_END
            D2_ELEM_END
            D2_ELEM(Text)
                D2_STYLE(ZIndex, 1)
                D2_STYLE(Value, "<X>")
                D2_STYLE(X, 1.0_pxi)
                D2_INTERP_TWOWAY_AUTO(Hovered, 500, Linear, ForegroundColor, d2::colors::r::crimson);
                D2_ON(Clicked)
                    state->core()->parent()->remove(state->core());
                D2_ON_END
            D2_ELEM_END
        D2_ELEM_END
        // Overview
            ...
            *metrics = {
            ...
            D2_ELEM(FlowBox)
                ...
                D2_ELEM(VerticalSwitch)
                    ...
                    D2_STYLE(
                        OnChangeValues,
                        [=](d2::TreeIter<VerticalSwitch>, d2::string n, d2::string o)
                        {
                            ...
                            if (f != metrics->end())
                            {
                                D2_CONTEXT_AUTO(output)
                                    if (!ptr->exists(n))
                                        D2_ELEM_STR(graph::CyclicVerticalBar, n)
                                            D2_STYLE(Width, 1.0_pc)
                                            D2_STYLE(Height, 1.0_pc)
                                        D2_ELEM_END
                                    else
                                    {
                                        ptr->at(n)->setstate(Element::Display, true);
                                    }
                                D2_CONTEXT_END
                                ...
                                *active_metric = {n, &f->second};
                                ...
                                gvalue->set<Text::Value>(
                                    std::format("Value: {}{}", f->second.measurement, f->second.unit)
                                );
                                minmax->set<Text::Value>(std::format(
                                    "Min/Max: [ {} {} ]{}", f->second.min, f->second.max, f->second.unit
                                ));
                            }
                            else
                            {
                ...
                D2_ELEM(ScrollFlowBox)
                    ...
                    auto option = [&](d2::string label, auto&& change, auto&& val, bool def = false)
                    {
                        D2_ELEM(Checkbox)
                            ...
                            D2_STYLE(
                                OnSubmit,
                                [=](d2::TreeIter<Checkbox>, bool value)
                                {
                            ...
                            (*metric_reloads).push_back(
                                [=]()
                                {
                                    ...
                                    {
                                        ptr.as<Checkbox>()->set<Checkbox::Value>(val(*active_metric->second));
                                    }
                                }
                            );
                        D2_ELEM_END
                    ...
                    option(
                        "track",
                        [=](const std::string& name, Metric& metric, bool value)
                        {
                            if (value)
                            {
                                metric.task = state->context()->scheduler()->launch_cyclic(
                                    metric.resolution,
                                    [&](auto&&...)
                                    {
                                        ...
                                        {
                                            gvalue->set<Text::Value>(std::format(
                                                "Value: {}{}",
            ...
            D2_ELEM(FlowBox)
                ...
                D2_ELEM(Text)
                    D2_STYLE(X, 1.0_px)
                    D2_STYLE(Y, 0.0_relative)
                    auto handle = state->context()->scheduler()->launch_cyclic(
                        std::chrono::milliseconds(200),
                        [=](auto&&...)
                        {
                            auto focused = ptr->screen()->focused();
                            auto info =
                                std::format("Focused: {}", focused == nullptr ? "<Unknown>" : focused->name());
            ...
            auto handle = state->context()->scheduler()->launch_cyclic(
                ...
                [=](auto&&...)
                {
                    auto update_mod_view = [=](TreeIter<> ptr, sys::SystemModule* mod)
                    {
                        ...
                        ptr.asp()->at("indicator").as<Text>()->set<Text::ForegroundColor>(color);
                    };
                    auto create_mod_view = [=](sys::SystemModule* mod)
                    {
                        D2_CONTEXT(ptr, ParentElement)
                            D2_ELEM_STR(FlowBox, mod->name())
                                ...
                                case sys::Load::Spec::Type::Deferred:
                                    load_type = std::format("Deferred ({})", mod->load_spec().ms);
                                    break;
                                }
                                D2_STYLE(Width, 1.0_pc)
                                D2_STYLE(Y, 0.0_relative)
                                ...
                                D2_ELEM(Text, indicator)
                                    ...
                                    D2_STYLE(X, 0.0_center)
                                D2_ELEM_END
                                D2_ELEM(Text, static-memory)
                                    D2_STYLE(Value, std::format("Static Memory: {}", impl::MeM(mod->static_usage())))
                                    D2_STYLE(Y, 0.0_relative)
                                D2_ELEM_END
                                D2_ELEM(Text, load)
                                    D2_STYLE(Value, std::format("Load: {}", load_type))
                                    D2_STYLE(Y, 0.0_relative)
                                D2_ELEM_END
                                D2_ELEM(Text, access)
                                    D2_STYLE(Value, std::format("Access: {}", mod->access() == sys::Access::TSafe ? "Thread Safe" : "Not Thread Safe"));
                                    D2_STYLE(Y, 0.0_relative)
                                D2_ELEM_END
                                D2_ELEM(Text)
                                    D2_STYLE(Value, "Dependencies:")
                                    D2_STYLE(Y, 0.0_relative)
                                D2_ELEM_END
        ...
        D2_ELEM(Box, Logs)
            ...
            D2_STYLE(Height, 1.0_pxi)
            D2_ELEM(MultiInput)
                D2_STYLE(InputOptions, MultiInput::ReadOnly)
                D2_STYLE(Width, 2.0_pxi) 
                D2_STYLE(Height, 1.0_pxi)
                D2_STYLE(X, 0.0_center)
                D2_STYLE(Y, 1.0_px)
                D2_STYLE(EnableScrollVert, true)
                D2_STYLE(EnableScrollHoriz, true)
                class Sink : public rs::RuntimeLogSink
                {
                public:
                    Sink(d2::TreeIter<MultiInput> ptr) : ptr(ptr) {}

                    d2::TreeIter<MultiInput> ptr;

                    virtual void accept(rs::LogEntry entry) noexcept override
                    {
                        if (ptr.weak().expired())
                            return;
                        ...
                        const auto synced = !scroll->getstate(d2::Element::Display) || scroll->relative_value() == 1.f;
                        ptr->append(value);
                        if (synced)
                            ptr->sync();
                    };
                ...
                ptr->disable_history();
                rs::context::get().sink<Sink>(ptr);
                Sink tmp(ptr);
                rs::context::get().page([&](rs::LogEntry entry) {
                    tmp.accept(entry);
                });
                D2_OFF_EXPR(Created, rs::context::get().unsink<Sink>());
            D2_ELEM_END
            ptr->setstate(Element::Display, false);
        D2_ELEM_END

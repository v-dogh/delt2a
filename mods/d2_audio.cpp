...
namespace d2::sys::ext
{
    ...
    SystemAudio::FilterPipeline
    SystemAudio::FilterPipeline::operator|(const std::string& name) const
    {
        FilterPipeline pipeline;
        pipeline.transforms.reserve(4);
    ...
    SystemAudio::FilterPipeline&
    SystemAudio::FilterPipeline::operator&&(std::function<bool(Stream)> callback)
    {
        if (transforms.empty() || !transforms.back().first.has_value() ||
    ...
    template<SystemAudio::Format> inline float _convert_sample(const unsigned char* sample);
    ...
    template<>
    inline float _convert_sample<SystemAudio::Format::Float32>(const unsigned char* sample)
    {
        return *reinterpret_cast<const float*>(sample);
    }
    template<>
    inline float _convert_sample<SystemAudio::Format::Float64>(const unsigned char* sample)
    {
    ...
    void _convert_from(
        ...
        SystemAudio::FormatInfo format,
        SystemAudio::FormatInfo expected
    )
    {
        constexpr auto sample_size = SystemAudio::sample_size(Fmt);

        if (format.channels == expected.channels && format.sample_rate == expected.sample_rate &&
        ...
        if (format.channels == expected.channels)
        {
            if (format.sample_rate > expected.sample_rate)
            {
                for (std::size_t i = 0; i < data.size();)
                {
                    for (std::size_t j = 0; j < expected.sample_rate; j += sample_size)
                    {
                        ...
                        out[j++] += SystemAudio::Stream::mix(a, b, base - float(std::size_t(base)));
                    }
            ...
            else if (format.sample_rate < expected.sample_rate)
            {
                for (std::size_t i = 0; i < data.size();)
                {
                    ...
                    for (std::size_t j = 0; j < expected.sample_rate; j += sample_size)
                    {
                        out[j++] += SystemAudio::Stream::mix(a, b, float(j) / expected.sample_rate);
                    }
                    i += format.sample_rate;
                }
            }
    ...
    std::size_t
    SystemAudio::_stream_normalize_required(std::span<const unsigned char> data, FormatInfo format)
    {
    ...
    void SystemAudio::_stream_normalize(
    ...
    )
    {
        ...
        {
        case Format::PCM16:
            _convert_from<Format::PCM16>(out, data, format, this->format(Device::Output));
            break;
        case Format::PCM32:
            _convert_from<Format::PCM32>(out, data, format, this->format(Device::Output));
            break;
        case Format::Float32:
            _convert_from<Format::Float32>(out, data, format, this->format(Device::Output));
            break;
        case Format::Float64:
            _convert_from<Format::Float64>(out, data, format, this->format(Device::Output));
            break;
        }
    }
    ...
    {
        _device_data[std::size_t(dev)].filter = std::move(filter);
    }
    ...
    void SystemAudio::_filter_override(

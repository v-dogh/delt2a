...
namespace d2::dx
{
    ...
    void Dropdown::_signal_write_impl(write_flag type, unsigned int prop, ptr element)
    {
        if (element.get() == this &&
            ...
            (prop == BorderWidth || prop == initial_property))
        {
    ...
    Unit Dropdown::_layout_impl(Element::Layout type) const
    {
        ...
        case Element::Layout::Height:
            ...
            {
                return Unit(_perfect_dimensions.height);
            }
            else
            {
                const auto bw = (data::container_options & data::EnableBorder)
                ...
                return Unit(int((height - bw) * cnt + bw) + data::height.raw());
            }
        }
    }
    ...
    void Dropdown::_event_impl(in::InputFrame& frame)
    {
        if (frame.had_event(in::Event::KeyInput))
        {
            ...
            else if (frame.active(in::special::Enter, in::mode::Press))
            {
    ...
    void Dropdown::_frame_impl(PixelBuffer::View buffer)
    {
        ...
        const auto yoff = (_cell_height / 2) + bw;
        {
            ...
            else
            {
                ...
                TextHelper::_render_text(
                    data::default_value, color, {xoff, yoff}, {bbox.width, _cell_height}, buffer
                );
            }
        }
        ...
        if (_opened)
        {
            for (std::size_t i = 0; i < data::options.size(); i++)
            {
                ...
                TextHelper::_render_text(
                    data::options[i],
                    (i == _soft_selected ? color_focused : color),
                    {xoff, yoff2},
                    {bbox.width, _cell_height},
                    buffer
                );
    ...
    void Dropdown::choose(int idx)
    {
        if (idx >= 0)
        {
            ...
            if (data::on_submit != nullptr)
                data::on_submit(
                    std::static_pointer_cast<Dropdown>(shared_from_this()), options[idx], idx
                );
        }
        _opened = false;
        _selected = idx;

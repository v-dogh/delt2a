#include "elements/d2_checkbox.hpp"
...
namespace d2::dx
{
    void Checkbox::_submit()
    {
        if (data::on_submit != nullptr)
            data::on_submit(
                std::static_pointer_cast<Checkbox>(shared_from_this()), data::checkbox_value
            );
    }
    ...
    void Checkbox::_state_change_impl(State state, bool value)
    {
        if (state == State::Clicked && value)
        {
            ...
            _submit();
            _signal_write(data::value_on.size() == data::value_off.size() ? Style : Masked);
        }
    }
    void Checkbox::_event_impl(in::InputFrame& frame)
    {
    ...
    void Checkbox::_frame_impl(PixelBuffer::View buffer)
    {
        const auto color = Pixel::combine(
            data::foreground_color, getstate(Keynavi) ? data::focused_color : data::background_color
        );
        buffer.fill(color);
        TextHelper::_render_text(
            data::checkbox_value ? data::value_on : data::value_off,
            data::checkbox_value
                ? Pixel::combine(data::color_on_foreground, data::color_on_background)
                : Pixel::combine(data::color_off_foreground, data::color_off_background),
            {0, 0},
            box(),
            buffer
        );

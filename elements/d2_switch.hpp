...
namespace d2
{
    ...
    namespace style
    {
        D2_UAI_INTERFACE(
            Switch,
            D2_UAI_OPTS(using Alignment = IZText::Alignment;),
            D2_UAI_FIELDS(
                // Ptr | new | old
            ...
            D2_UAI_LINK(
                D2_UAI_PROP(Options, options, Masked) D2_UAI_PROP(
                    EnabledForegroundColor, enabled_foreground_color, Style
                    ...
                    D2_UAI_PROP(DisabledForegroundColor, disabled_foreground_color, Style)
                        D2_UAI_PROP(DisabledBackgroundColor, disabled_background_color, Style)
                            D2_UAI_PROP(OnChange, on_change, 0x00) D2_UAI_PROP(
                                OnChangeValues, on_change_values, 0x00
                            ) D2_UAI_PROP(SeparatorColor, separator_color, Style)
                                D2_UAI_PROP(DisableSeparator, disable_separator, Style) D2_UAI_PROP(
                                    ExpandBackground, disable_separator, Style
        ...
        D2_UAI_INTERFACE(
            ...
            D2_UAI_PROPS(SliderBackgroundColor, SliderColor, LeftConnector, RightConnector),
            D2_UAI_LINK(D2_UAI_PROP(SliderBackgroundColor, slider_background_color, Style)
    ...
    namespace dx
    {
        ...
        class VerticalSwitch : public style::UAIC<Switch, VerticalSwitch, style::IVerticalSwitch>
        {
        public:
            using data = style::UAIC<Switch, VerticalSwitch, style::IVerticalSwitch>;
            D2_UAI_CHAIN(Switch)
        private:
        ...
        protected:
            ...
            virtual void _frame_impl(PixelBuffer::View buffer) override;
        public:
            VerticalSwitch(const std::string& name, TreeState::ptr state) : data(name, state)
            {

...
namespace d2
{
    namespace style
    {
        D2_UAI_INTERFACE(
            ...
            D2_UAI_OPTS(),
            D2_UAI_FIELDS(
                pixel max_color{.a = 0, .v = '.'}; pixel min_color{.a = 0, .v = '.'};
                int resolution{0};
            ...
            D2_UAI_PROPS(
                MaxColor, MinColor, Resolution, AutomaticResolution, HighResolution, Invert
            ),
            D2_UAI_LINK(D2_UAI_PROP(MaxColor, max_color, Style)
        ...
        D2_UAI_INTERFACE(
            ...
            D2_UAI_FIELDS(
                ...
                px::foreground node_color_i{px::foreground(colors::g::green)};
                px::foreground node_color_a{px::foreground(colors::b::blue)};
                px::foreground node_color_b{px::foreground(colors::r::red)};
                px::foreground node_color_focused{px::foreground(colors::g::olive)};
            ),
            D2_UAI_PROPS(
                Resolution, Frequency, NodeColorI, NodeColorA, NodeColorB, NodeColorFocused
            ),
            D2_UAI_LINK(D2_UAI_PROP(Resolution, resolution, Style) D2_UAI_PROP(
                Frequency, frequency, Style
    ...
    namespace dx::graph
    {
        class CyclicVerticalBar : public style::
                                      UAI<CyclicVerticalBar,
                                          style::ILayout,
                                          style::IContainer,
                                          style::IColors,
                                          style::IRangeGraph>,
        ...
        class NodeWaveGraph : public d2::style::
                                  UAI<NodeWaveGraph,
                                      d2::style::ILayout,
                                      d2::style::IColors,
        ...
        public:
            ...
            struct NodePair
            {
                ...
                std::function<float(float, float, float, std::span<const float>)> interpolator{
                    [](float phase, float frequency, float pos, auto) { return 0.f; }
                };
            };
        private:

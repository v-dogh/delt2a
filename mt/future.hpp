...
namespace mt
{
    ...
    template<typename Type, template<typename> typename Block> class Future
    {
    public:
    ...
    public:
        template<typename... Argv> static auto make(Argv&&... args)
        {
            return Future(std::make_shared<Block<Type>>(std::forward<Argv>(args)...));
        }
        ...
        bool await_suspend(std::coroutine_handle<> handle)
        {
            ...
            return !(!_block->try_set_handle(handle) || (has_value() && _block->has_handle()));
        }
        Type await_resume()
        {
            return value();
    ...
    template<typename Type> using future = Future<Type, ControlBlock>;

    template<typename Type> using man_future = Future<Type, ManagedControlBlock>;

    template<typename Type> using co_future = Future<Type, CoroutineControlBlock>;

    template<typename Base, typename Type> struct PromiseBase
    {
        template<typename... Argv> void return_value(Argv&&... args) noexcept
        {
            if (!static_cast<Base*>(this)->_block)
                return;
            static_cast<Base*>(this)->_block->set_value(std::forward<Argv>(args)...);
        }
    };
    template<typename Base> struct PromiseBase<Base, void>
    {
        void return_void() noexcept
        {
            if (!static_cast<Base*>(this)->_block)
                return;
            static_cast<Base*>(this)->_block->set_value();
        }
    };

    template<typename Type> class Promise : public PromiseBase<Promise<Type>, Type>
    {
    public:
    ...
    public:
        ...
        auto final_suspend() noexcept
        {
            struct awaitable
            {
                ...
                {
                    return !(
                        !block->try_set_handle(handle) ||
                        (block->has_value() && block->has_handle())
                    );
                }
    ...
    template<typename Base> class DummyPromise
    {
    public:

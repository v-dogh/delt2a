...
namespace mt
{
    template<typename Pool> struct DeferDiscard : Pool
    {
    ...
    template<typename Pool> struct DeferDecision
    {
        using defer_type = DeferDiscard<Pool>;

        template<typename... Argv> static auto defer(Argv&&... args)
        {
    ...
    struct PoolDefer
    {
        ...
        PoolDefer(ThreadPool::ptr ptr) : pool(ptr) {}

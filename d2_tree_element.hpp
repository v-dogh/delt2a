...
namespace d2
{
                    ...
                    public style::UniversalAccessInterfaceBase
    {
        D2_TAG_MODULE(tree)
    public:
        template<typename Type> using eptr = std::shared_ptr<Type>;
        ...
        public:
            explicit Frame(cptr obj) : _object(obj) {}
            Frame(Frame&&) = default;
        ...
        using state_flag = unsigned short;
        ...
        enum InternalState : internal_flag
        {
            ...
            // Bypasses any caching techniques
            CachePolicyBypass = 1 << 11,
        };
    ...
    private:
        ...
        public:
            ...
            EventListenerState(
                Element::ptr ptr, std::size_t idx, Dep value, State ev, Func callback
            ) :
                _obj(ptr), _idx(idx), _value(value), _event(ev),
            ...
            template<typename... Argv> void invoke(Argv&&... args);
    ...
    public:
        ...
        public:
            ...
            EventListener(EventListenerState::ptr ptr) : _ptr(ptr) {}
    ...
    private:
        ...
        mutable std::atomic<internal_flag> _internal_state{
            WasWritten | WasWrittenLayout | IsBeingInitialized | CachePolicyBypass
            // CachePolicyStatic
            // CachePolicyDynamic
            // CachePolicyVolatile
        };
        ...
        std::size_t _depth{0};
        std::size_t _frames_reused{0};
        std::size_t _frames_redrawn{0};
        in::InputFrame* _tmp_frame{nullptr};
    ...
    private:
        ...
        EventListener
        _push_listener(State event, EventListenerState::Dep value, event_callback callback);
        ...
        void
        _signal_write_internal(write_flag type = WriteType::Masked, unsigned int prop = ~0u) const;
        ...
        void _reset_depth();
        void _setparent(pptr parent);
    protected:
        // Write handling
        // Context changes occur when a parent element changes it's layout and a child has relative
        // layout Then we notify it to recompute it's values for the layout Layout writes signal
        // that the bounding box and position of an object changed Style writes signal that either
        // the framebuffer itself, or it's contents changed (for an element) For example a resize is
        // both a Style and Layout write since it requires a resize of the buffer and changes the
        // box
    ...
    protected:
        ...
        void _trigger_event(in::InputFrame& frame, bool recursive);
    protected:
        // Layout
        ...
        virtual bool _provides_input_impl() const
        {
            return false;
        }
        ...
        virtual void _event_impl(in::InputFrame&) {}
        ...
        virtual bool _provides_buffer_impl() const
        {
            return false;
        }
    ...
    public:
        ...
        static auto make(const std::string& name, TreeState::ptr state, Argv&&... args)
        {
            auto ptr = std::make_shared<Type>(name, state, std::forward<Argv>(args)...);
        ...
        Element() = default;
        Element(const std::string& name, TreeState::ptr state) : _name(name), _state_ptr(state) {}
        ...
        in::InputFrame& input_frame() const;
        ...
        EventListener listen(State event, event_callback callback);
    ...
    template<typename Base, Element::CachePolicy Policy> class CacheDef
    {
    public:
        CacheDef()
        {
    ...
    namespace internal
    {
        ...
        public:
            ...
            void signal_context_change_sub(
            ...
            void signal_context_change(
                Element::write_flag type, unsigned int prop, Element::ptr element
            );
            void signal_context_change(
                Element::write_flag type = Element::WriteType::Masked, unsigned int prop = ~0u
            );
            void signal_write(
                Element::write_flag type = Element::WriteType::Masked, unsigned int prop = ~0u
            );

...
namespace d2
{
    int Element::LayoutStorage::get(enum Layout comp) const noexcept
    {
        return _storage[static_cast<std::size_t>(comp)];
    }
    void Element::LayoutStorage::set(enum Layout comp, int value) noexcept
    {
    ...
    std::shared_ptr<Element> TreeIter::operator->() const noexcept
    {
        return _elem.lock();
    }
    std::shared_ptr<Element> TreeIter::operator->() noexcept
    {
        return _elem.lock();
    }

    Element& TreeIter::operator*() const noexcept
    {
        return *_elem.lock();
    }
    Element& TreeIter::operator*() noexcept
    {
        return *_elem.lock();
    }
    ...
    void Element::_signal_write_local(write_flag type, unsigned int prop, ptr element) noexcept
    {
        if ((type & ~(WriteType::Style | WriteType::InternalLayout)) & WriteType::Dimensions)
        {
            ...
            if (_contextual_change(pos))
                _invalidate_state(pos);
        }
    ...
    Element::pptr Element::parent() const noexcept
    {
        return _parent.lock();
    }
    ...
    int Element::resolve_units(Unit unit) const noexcept
    {
        [[ unlikely ]] if (!parent())
            return unit.raw();
    ...
    int Element::layout(enum Layout type) const noexcept
    {
        switch (type)
        {
            case Layout::X:
    ...
    Unit Element::internal_layout(enum Layout type) const noexcept
    {
        return _layout_impl(type);
    }

    void Element::accept_layout() const noexcept
    {
        _internal_state |= (PositionXUpdated | PositionYUpdated | DimensionsWidthUpdated | DimensionsHeightUpdated);
    }
    void Element::accept_layout(enum Layout type) const noexcept
    {
        switch (type)
        {
    ...
    void Element::override_layout(enum Layout type, int value) const noexcept
    {
        _layout.set(type, value);
        accept_layout(type);
    }
    void Element::override_dimensions(int width, int height) const noexcept
    {
    ...
    void Element::override_position(int x, int y) const noexcept
    {
    ...
    bool Element::contextual_layout(enum Layout type) const noexcept
    {
        const unit_meta_flag rep = _unit_report_impl();
        switch (type)
        {
    ...
    bool Element::relative_layout(Layout type) const noexcept
    {
        const unit_meta_flag rep = _unit_report_impl();
        switch (type)
        {
    ...
    Element::Position Element::mouse_object_space() const noexcept
    {
        if (parent())
        {
    ...
    TreeIter Element::traverse() noexcept
    {

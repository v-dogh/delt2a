...
namespace d2::interp
{
	...
	protected:
		...
		{
			return
				float(std::chrono::duration_cast<std::chrono::milliseconds>((std::chrono::high_resolution_clock::now() - start_)).count()) /
		...
		virtual std::size_t _hash_code_impl() noexcept = 0;
	public:
		...
		void setowner(std::size_t hash) noexcept
		{
			owner_ = hash;
		}
		std::size_t getowner() noexcept
		{
			return owner_;
		}
	};
	...
	protected:
		virtual bool _keep_alive_impl(float progress) noexcept override
		{
			return progress < 1.f;
		}
		...
		virtual std::size_t _hash_code_impl() noexcept override
		{

...
namespace d2::ctm
{
	...
	void FilesystemExplorer::forwards()
	{
		if (!history_.empty())
		{
	...
	void FilesystemExplorer::backwards()
	{
	...
	void FilesystemExplorer::setpath(std::filesystem::path path)
	{
		const auto [ width, height ] = box();
	...
	std::filesystem::path FilesystemExplorer::getpath() const noexcept
	{
		return path_;
	}

	void FilesystemExplorer::submit_soft()
	{
	...
	void FilesystemExplorer::submit()
	{
		if (data::on_submit != nullptr)
			data::on_submit(shared_from_this());
		close();

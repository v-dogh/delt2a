...
namespace d2::dx
{
	void Box::_recompute_layout(int basex, int basey) const noexcept
	{
	...
	bool Box::_is_automatic_layout() const noexcept
	{
		return !(data::container_options & ContainerOptions::DisableAutoLayout);
	}

	Box::Position Box::_position_for(cptr elem) const
	{
	...
	Box::BoundingBox Box::_dimensions_for(cptr elem) const
	{
	...
	bool Box::_provides_layout_impl(cptr elem) const noexcept
	{
		return elem->relative_position() && _is_automatic_layout() && elem->getstate(Display);
	}
	bool Box::_provides_dimensions_impl(cptr elem) const noexcept
	{
		return elem->relative_dimensions() && _is_automatic_layout();
	}

	int Box::_resolve_units_impl(Unit value, cptr elem) const noexcept
	{
	...
	char Box::_index_impl() const noexcept
	{
		return data::zindex;
	}
	Box::unit_meta_flag Box::_unit_report_impl() const noexcept
	{
		return UnitUpdateHelper::_report_units();
	}

	Box::BoundingBox Box::_dimensions_impl() const noexcept
	{
	...
	Box::Position Box::_position_impl() const noexcept
	{
		return {
	...
	void Box::_fleshbox_object_render(PixelBuffer::View buffer, ptr obj)
	{
		// Mezon the monstrous
		// It is high time

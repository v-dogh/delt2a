...
namespace d2::lsx
{
    namespace impl
    {
        void Row::substitute(Symbol lhs, const Row& rhs) noexcept
        {
        ...
        std::optional<Symbol> Row::find(Symbol symbol) noexcept
        {
        ...
        void Row::insert(Symbol term) noexcept
        {
            _terms.emplace(Symbol(term, term.coefficient() / _lazy_terms_coefficient));
        }
        void Row::erase(Symbol term) noexcept
        {
            _terms.erase(term);
        }

        void Row::reserve(std::size_t count) noexcept
        {
            _terms.reserve(count);
        }

        bool Row::contains(Symbol term) noexcept
        {
            return _terms.contains(term);
        }

        const result_type& Row::constant() const noexcept
        {
            return _constant;
        }
        result_type& Row::constant() noexcept
        {
            return _constant;
        }
        ...
        std::string Row::print() const noexcept
        {
        ...
        Row& Row::operator*=(result_type cf) noexcept
        {
    ...
    std::optional<Symbol> ConstraintArray::_find_subject(const impl::Row& row) noexcept
    {
    ...
    std::optional<Symbol> ConstraintArray::_find_entering() noexcept
    {
    ...
    std::optional<Symbol> ConstraintArray::_find_dual_entering(tableau_iterator row) noexcept
    {
    ...
    std::optional<ConstraintArray::tableau_iterator> ConstraintArray::_find_leaving(Symbol entering) noexcept
    {
    ...
    std::optional<VariableIndex> ConstraintArray::_pop_infeasible() noexcept
    {
    ...
    void ConstraintArray::_pivot(Symbol entering, tableau_iterator leaving) noexcept
    {
    ...
    void ConstraintArray::_mark(VariableIndex symbol, bool condition) noexcept
    {
        if (condition) _infeasible.emplace(symbol);
        else _infeasible.erase(symbol);
    }

    bool ConstraintArray::_dual_optimize() noexcept
    {
    ...
    bool ConstraintArray::_optimize() noexcept
    {
    ...
    void ConstraintArray::_set_error(Variable error, result_type strength) noexcept
    {
        const auto [ it, _ ] = _tableau.emplace(VariableIndex(Symbol::Type::Objective), impl::Row());
        it->second.insert(Variable(error, strength));
    }
    void ConstraintArray::_set_error(Variable e1, Variable e2, result_type strength) noexcept
    {
        const auto [ it, _ ] = _tableau.emplace(VariableIndex(Symbol::Type::Objective), impl::Row());
    ...
    std::string ConstraintArray::status_to_string(Status status) noexcept
    {
        switch (status)
        {
    ...
    ConstraintArray::Status ConstraintArray::status(result_type value) const noexcept
    {
    ...
    bool ConstraintArray::remove_constraint(handle handle) noexcept
    {
        return false;
    }
    ConstraintArray::handle ConstraintArray::set_constraint(const Expression& expr, Strength strength) noexcept
    {
        return set_constraint(expr, result_type(strength));
    }
    ConstraintArray::handle ConstraintArray::set_constraint(const Expression& expr, result_type strength) noexcept
    {
    ...
    result_type ConstraintArray::get(Variable var) const noexcept
    {
    ...
    void ConstraintArray::start_batch() noexcept
    {
        _flags |= IsBatched;
    }
    void ConstraintArray::end_batch() noexcept
    {
        _flags |= ~IsBatched;
    }

    void ConstraintArray::clear() noexcept
    {
        _tableau.clear();
        _infeasible.clear();
        _internal_id_ctr = 0;
    ...
    std::string ConstraintArray::print() const noexcept
    {
        std::stringstream out;
        out << "Tableau:\n";
        for (decltype(auto) it : _tableau)

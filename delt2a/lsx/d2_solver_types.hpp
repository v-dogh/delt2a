...
namespace d2::lsx
{
    ...
    public:
        ...
        constexpr bool is_derived() const noexcept
        {
        ...
        constexpr bool is_pivotable() const noexcept
        {
            const auto tp = type();
            return
                tp == Type::Slack ||
        ...
        constexpr std::uint32_t code() const noexcept
        {
            return _code;
        }
        ...
        bool operator==(const VariableIndex& copy) const noexcept
        {
    ...
    public:
        ...
        constexpr result_type coefficient() const noexcept
        {
            return _coefficient;
        }

        constexpr Variable operator[](std::size_t idx) const noexcept
        {
    ...
    public:
        ...
        constexpr Symbol invert() const noexcept
        {
        ...
        constexpr const Symbol& operator*=(result_type cf) const noexcept
        {
            _coefficient *= cf;
            return *this;
        }
    ...
    public:
        ...
        static constexpr Expression reduce(const Expression& lhs, const Expression& rhs, Constraint op) noexcept
        {
            Expression result;
            result._op = op;
        ...
        static constexpr Expression from(Variable lhs, Variable rhs, int sign) noexcept
        {
            Expression result;
            result._terms[0] = lhs;
        ...
        static constexpr Expression from(const Expression& lhs, Variable rhs, int sign) noexcept
        {
            Expression result;
        ...
        static constexpr Expression from(Variable lhs, const Expression& rhs, int sign) noexcept
        {
            return from(rhs, lhs, -sign);
        }
        static constexpr Expression from(const Expression& lhs, result_type cf) noexcept
        {
            Expression result;
        ...
        static constexpr Expression from(result_type cf, const Expression& rhs, int sign) noexcept
        {
            return from(rhs, cf, -sign);
        }
        static constexpr Expression from(Variable var) noexcept
        {
            Expression result;
            result._terms[0] = var;
            result._size_vars++;
        ...
        constexpr Constraint type() const noexcept
        {
            return _op;
        }

        constexpr std::span<const Variable> vars() const noexcept
        {
            return std::span(_terms.begin(), _terms.begin() + _size_vars);
        }
        constexpr std::span<const Variable> locals() const noexcept
        {
            return std::span(_terms.begin() + _size_vars, _terms.begin() + _size_vars + _size_locals);
        }
        constexpr std::span<const Variable> globals() const noexcept
        {
            return std::span(_terms.begin() + _size_vars + _size_locals, _terms.begin() + _size_vars + _size_globals);
        }

        constexpr result_type constant() const noexcept
        {
            return _constant;
        }
    ...
    {
        constexpr Variable operator[](Variable var) const noexcept
        {
            return Variable(
...
namespace std
{
    ...
    {
        std::size_t operator()(const d2::lsx::VariableIndex& idx) const noexcept
        {
    ...
    {
        std::size_t operator()(const d2::lsx::Symbol& idx) const noexcept
        {
            return std::hash<std::uint32_t>()(idx.code());
        }
    };

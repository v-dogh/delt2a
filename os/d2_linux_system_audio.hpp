...
namespace d2::sys::ext
{
    ...
    private:
        ...
        static DeviceName&
        _add_device(std::vector<DeviceName>& v, const std::string& id, const std::string name);
        ...
        void
        _event_metadata_impl(uint32_t id, const char* key, const char* type, const char* value);
        ...
        template<typename Func, typename... Argv> void _run_this(Func&& callback, Argv&&... args)
        {
            ...
            else
            {
                std::tuple block = std::make_tuple(
                    std::forward<Func>(callback), this, std::make_tuple(std::forward<Argv>(args)...)
                );
        ...
        void _run_this_async(Func&& callback, Argv&&... args)
        {
            ...
            {
                auto* block =
                    new std::tuple{std::forward<Func>(callback), this, std::make_tuple(args...)};
                while (true)
                {
                    auto result = pw_loop_invoke(
                        ...
                        {
                            auto* arguments =
                                static_cast<std::remove_cvref_t<decltype(block)>>(data);
                            std::apply(
                                [&](auto&&... args)
                                {
                                    std::invoke(
    ...
    public:
        ...
        virtual void activate(
            Device dev, const std::string& name = "", DeviceConfig cfg = DeviceConfig()
        ) override;
        ...
        virtual void
        filter_push(Device dev, const std::string& after, FilterPipeline filter) override;

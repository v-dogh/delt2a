...
namespace d2::lsx
{
    std::string VariableIndex::print() const
    {
        ...
        return is_derived() ? std::format("{}_{}^", mark, index())
                            : std::format("{}_{}", mark, index());
    }
    ...
    std::string Expression::print() const
    {
        ...
        {
            if (off || _terms[off].coefficient() < 0.f)
            {
                out << (_terms[off].coefficient() < 0.f ? " - " : " + ");
            }
            out << std::abs(_terms[off].coefficient()) << "V[" << _terms[off].index() << "]";
        }
        for (std::size_t i = 0; i < _size_locals; i++, off++)
        {
            ...
            out << std::abs(_terms[off].coefficient()) << "L[" << _terms[off].index() << "]";
        }
        for (std::size_t i = 0; i < _size_globals; i++, off++)
        {
            ...
            out << std::abs(_terms[off].coefficient()) << "G[" << _terms[off].index() << "] + ";
        }

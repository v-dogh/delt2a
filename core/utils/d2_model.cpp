...
namespace d2
{
    ...
    void MatrixModel::load_model(const std::string& path, ModelType type)
    {
        ...
        if (type == ModelType::Raw)
        {
            ...
            auto off = 1;
            width =
                byteswap_arithmetic_if(*reinterpret_cast<const std::uint32_t*>(data.data() + off));
            ...
            else
            {
                ...
                {
                    buffer.push_back(*reinterpret_cast<const pixel*>(&data[i]));
                }
            }
        }
        else if (type == ModelType::Visual)
        {
            ...
            while (idx < data.size())
            {
                ...
                {
                    buffer.push_back(pixel{.v = data[idx]});
                }

                lwidth = width - lwidth;
                while (lwidth)
                {
                    buffer.push_back(pixel{.v = ' '});
                    lwidth--;
                }
    ...
    void MatrixModel::save_model(const std::string& path, ModelType type)
    {
        ...
        if (type == ModelType::Raw)
        {
            const std::uint8_t header = (std::endian::native == std::endian::little) * Endian;
            ...
            file.write(
                reinterpret_cast<const char*>(buffer_.data()), buffer_.size() * sizeof(pixel)
            );
        }
        else if (type == ModelType::Visual)
        {

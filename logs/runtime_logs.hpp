...
namespace rs
{
    enum class Severity : unsigned char
    {
        ...
        Module = 1 << 4,
        // Information that signals not an error but a possible misconfiguration or unintended
        // effect
        Warning = 1 << 5,
        // A controlled error during the lifetime of an operation
        Error = 1 << 6,
        // An uncontrolled error during the lifetime of an operation (e.g. an out of memory
        // exception)
        Critical = 1 << 7,
    };
    ...
    private:
        template<typename Type> static void _write(void* buffer, Type&& value)
        {
            std::memcpy(buffer, reinterpret_cast<const unsigned char*>(&value), sizeof(Type));
        }
        template<typename Type> static void _write(void* buffer, Type&& value, std::size_t& ctr)
        {
            std::memcpy(
        ...
        struct LogHeader
        {
            union
            {
                struct
                {
                    std::chrono::steady_clock::time_point
                        timestamp{};           // timestamp relative to baseline
            ...
            static LogHeader from(const unsigned char* buffer);
        };
        struct DumpHeader
        {
    ...
    public:
        ...
        void page(std::function<void(LogEntry)> callback, std::size_t cnt) const;
        void page_epoch(
        ...
        RuntimeLogs& logd(
            Severity severity, std::string_view module, std::string_view data, Argv&&... args
        ) noexcept
        {
            if (!(unsigned(severity) & unsigned(_filter.load())))
            {
                ((in << rs::log(severity, module) << rs::dat(data))
...
#define LOG(_severity_, _mod_, ...)                                                                \
    ::rs::context::get().log(::rs::Severity::_severity_, #_mod_ __VA_OPT__(, ) __VA_ARGS__);
#define LOGD(_severity_, _mod_, _data_, ...)                                                       \
    ::rs::context::get().log(::rs::Severity::_severity_, #_mod_, _data_ __VA_OPT__(, ) __VA_ARGS__);
    ...
    namespace ex
    {
        ...
        auto safe(
            ...
            -> std::optional<std::conditional_t<
                std::is_same_v<decltype(callback(args...)), void>,
                Dummy,
